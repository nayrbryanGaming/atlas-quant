...
import { exitPosition } from '@/core/pilot/engine';
...
export async function POST(request: Request) {
    try {
        ...
        if (!signal) {
            return NextResponse.json({ error: 'No cached signal available to derive exit price' }, { status: 400 });
        }

        const state = exitPosition(userKey, symbol, signal.price);
        return NextResponse.json({ success: true, state });
    } catch (error: any) {
        logger.warn('Pilot Exit Failure', error);
